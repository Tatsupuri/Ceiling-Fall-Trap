...
public class DinasaurGenerator : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if(timer >= rate)
        {
            dir = Quaternion.LookRotation(GameObject.FindWithTag("MainCamera").transform.position, Vector3.up);
            timer = 0f;
            //,dir
            //GameObject dinasaur = Instantiate(dinasaurPrefab,gate.transform);
            GameObject dinasaur = Instantiate(dinasaurPrefab,this.transform.parent.transform.position, this.transform.parent.transform.rotation);
